...
import {ActivityIndicator,ImageBackground, Button, Text, TouchableOpacity, View, StyleSheet, Image} from 'react-native';
...
const SplashScreen = ({navigation}) => {
  ...
  const fetchUserData = (userId) => {
    console.log("Console of Splsh Screen: "+ JSON.parse(navigation));
    if(userId){
  ...
  return (
    <View  style={{flex:1,width:'100%',paddingHorizontal:1,height:'100%'}}>
      <ImageBackground
      resizeMode="stretch"
      style={{flex:1,width:'100%',justifyContent:'center',alignItems: 'center',position:'relative',height:'100%'}}
      source={require('../../assets/s1bg.jpg')}
      >
      <Image  source={require('../../assets/s1img.png')} resizeMode='contain' style={{width:'85%',height:'55%'}} />

      <View style={{width:'100%',alignItems:'center',marginTop:10}}>
        <Text style={{color:'black',fontSize:26,}}>Make Order</Text>
      </View>
      <View style={{width:'90%',alignItems:'center',justifyContent: 'center',paddingHorizontal:'8%'}}>
        <Text style={{color:'grey',fontSize:16,textAlign:'center'}}>Get different variety of services from our trusted partners </Text>
      </View>
      ...
      <View style={{position:'absolute', bottom:20,right:20,flexDirection:'row'}}>
        <TouchableOpacity style={{ right:10,flexDirection:'row'}} onPress={()=>navigation.navigate('SplashScreenSecond')} >
